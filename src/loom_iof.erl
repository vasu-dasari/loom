...
find_switch(Key) when is_integer(Key) ->
    case lists:keyfind(Key, 1, loom_logic:switches()) of
...
connect(IpAddr, Port) ->
    loom_logic:connect(IpAddr, Port).
...
disconnect(Key) ->
    loom_logic:close_connection(Key).
...
switches() ->
    ...
    [io:format("~1s~-9.10B ~-32s ~-17w ~-7.10B~n",
        ...
        {SwitchKey, DatapathId, IpAddr, Version, _} <-
            lists:sort(loom_logic:switches())],
    ok.
...
%% @end
version(Key) ->
    check_version(loom_logic:ofs_version(Key)).
